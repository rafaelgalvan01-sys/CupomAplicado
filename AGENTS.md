...
- **Tabela nova (não só coluna nova) tem um código de erro DIFERENTE pra detectar no fallback.** `42703` é o erro cru do Postgres pra coluna ausente, mas pra tabela ausente a API REST da Supabase (PostgREST) intercepta ANTES de chegar no Postgres e devolve o código próprio dela, `PGRST205` ("Could not find the table ... in the schema cache") — não o `42P01` (undefined_table) que seria o esperado se fosse erro cru do Postgres. Confirmado ao vivo ao criar a tabela `guides` (migração 0010): `select('*')` numa tabela que ainda não existe quebra a página inteira (500) até eu trocar o `error.code === '42P01'` por `error.code === 'PGRST205'` no guard de `lib/data.ts`. Testar sempre contra o servidor real antes de assumir qual código vem — não adivinhar pela memória do código Postgres.
